...
macOS menu bar companion app that attaches a floating side panel and design overlays to the iOS Simulator. Built with AppKit + SwiftUI, on Apple frameworks plus two approved SPM exceptions (Pulse/PulseProxy, linked by both the app and the in-Simulator companion framework, and Sparkle for auto-update — see [Dependencies](#dependencies)).
...
- Build Stats — recent build times charted from Xcode DerivedData
- AX Inspector — live accessibility tree browsing with element highlighting
...
All App Actions sections — including the four above — are searchable from the Actions tab's quick search (18 actions across 13 sections).
...
BoosterSimApp is distributed outside the Mac App Store, as a **notarized, stapled DMG** published on [GitHub Releases](https://github.com/phuongddx/BoosterSimApp/releases):

- **Developer ID signed + notarized + stapled** — Gatekeeper verifies the app at first launch with no network check
- **Auto-update via [Sparkle](https://sparkle-project.org)** — **Check for Updates…** in the menu bar item; the app watches the [appcast feed](https://github.com/phuongddx/BoosterSimApp/releases/latest/download/appcast.xml)
- One command reproduces a release locally — `scripts/build-release.sh` (archive → Developer-ID export → notarize → staple → DMG), described in the [deployment guide](docs/deployment-guide.md)

> The first public release is pending the one-time notarization and signing-key setup in the deployment guide; until it lands, build from source below. The current app icon is a scripted placeholder (amber `#E8720C` + white bolt, all 10 sizes, regenerated by `scripts/generate-placeholder-icon.swift`) pending real design.
...
Sparkle stays app-target-only by platform — a macOS auto-update framework must not link into the iphonesimulator `BoosterSimConnect` target. It powers the **Check for Updates…** menu item (`SPUStandardUpdaterController` in `AppDelegate`) and verifies appcast updates against the EdDSA key in `SUPublicEDKey`. The pin set is locked in the tracked `Package.resolved` (pulse 5.2.2 + sparkle 2.9.6).
...
Four Xcode targets. `BoosterSimApp` is 128 Swift files (~16,129 LOC), `BoosterSimConnect` is 6 (~772 LOC), and the two test bundles are 32 files (~4,296 LOC). `PrivacyInfo.xcprivacy` ships in the app bundle's Resources.
...
└── AppDelegate                     — composition root, wires everything
    ...
    └── SPUStandardUpdaterController — Sparkle auto-update (Check for Updates…)
