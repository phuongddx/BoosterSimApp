...
- [Phase 07]: 07-REVIEW.md (deep, 27 files, empirical probes against the real Sparkle 2.9.6 binary): 2 CRITICAL findings in release.yml's appcast step (wrong SPM artifact path, invalid generate_appcast invocation — every tag push would have died) both fixed and proven against the real binary; WR-01 (Sparkle updater never actually starts — lazy var never touched) and WR-02 (deployment guide's EdDSA paste site was wrong) fixed; WR-03 (FileTimestamp reason codes C617.1+3B52.1 were wrong per Apple's current docs — 3B52.1 is third-party-SDK-only) fixed post-review to DDA9.1 alone, DerivedData mtime reads left honestly uncovered rather than padded
- [Phase 07][deferred]: WR-04 — CameraService.probeSupport has no ordering guard; a stale probe from a fast simulator-switch can overwrite a newer simulator's camera state. Fix sketch in 07-REVIEW.md (generation-token / lastProbedPID check before the state write). Belongs to the Phase 6 service owner; needs a regression test with a scripted double. Not phase-blocking (Phase 6's original single-trigger design made the race window narrow; Phase 7's added onChange trigger widens it).
...
- [infra][resolved 2026-09-01]: Package.resolved tracked in git for the first time (07-02) — closes the vacuous-pin-check gap carried since Phase 2; 07-06's git diff --exit-code assertion is real now (pins: pulse 5.2.2 + sparkle 2.9.6).
